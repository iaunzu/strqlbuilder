...
import com.github.iaunzu.strqlbuilder.hibernate.SqmTranslator;
...
public class StrQLBuilder {
	...
	/**
	 * Appends a <code>UNION</code> operator and returns a new {@link StrQLBuilder}
	...
	private <X> StrTypedQuery<X> createQuery(EntityManager entityManager, Class<X> clazz, boolean pagedCount) {
		...
		if (fixJpqlCountQuery) {
			// always native
			String nativeSelect = new SqmTranslator(entityManager).translateNativeSelect(sql);
			sql = "select count(*) from (" + nativeSelect + ") x";
		}

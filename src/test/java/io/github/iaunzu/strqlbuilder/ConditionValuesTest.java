...
import io.github.iaunzu.strqlbuilder.utils.ConditionValuesParser;
...
public class ConditionValuesTest {
    ...
    public void shouldSplitInWithAnotherParam() {
        ConditionValues cv1 =
                new ConditionValues("(x.id not in (:list)) and x.id = :id", Arrays.asList(10L, 20L, 30L, 40L), "A");
        ConditionValues cv = new ConditionValuesParser(cv1, 3).parse();
    ...
    public void fullTest() {
        String condition =
                "(x.id in (:list) and x.id in (2) or (x.id not in (:list_2)) and x.id = :id) and (1<>2 or x.id in :list3)";
        ConditionValues cv1 = new ConditionValues(
                condition, //
        ...
        assertThat(
                cv.getCondition(),
                is("((x.id in (:list__0) or x.id in (:list__1)) "
